...
def address_parser(text: Optional[str], org_name: Optional[str] = None) -> Optional[List[str]]:
    ...
    # 동/읍/면 패턴 추출 (text)
    address_pattern = r'(\b\w+동|\b\w+읍|\b\w+면|\b\w+가)'
    ...
    if org_name:
        ...
        #1,2,3등 숫자 붙은 동도 추가
        unique_addresses.extend([re.sub(r'\d+', '', match) for match in matches2])
        unique_addresses.extend([re.sub(r'\d+', '', match) for match in org_matches])
