...
def get_region_maps_from_db() -> Tuple[Dict[str, int], Dict[str, List[Dict[str, int]]], Dict[str, List[Dict[str, int]]], Dict[int, Any]]:
    """DB에서 지역 정보를 조회하여 4가지 종류의 맵을 생성합니다."""
    ...
    try:
        ...
        sido_map, all_sigungus_map, all_eupmyeondong_map, id_to_region_map = {}, {}, {}, {}
        ...
        for region in regions:
            ...
            elif region.region_level == 3:
                if region.region_name not in all_eupmyeondong_map:
                    all_eupmyeondong_map[region.region_name] = []
                all_eupmyeondong_map[region.region_name].append({'id': region.id, 'parent_id': region.parent_id})
        
        print(f"✅ {len(sido_map)}개의 시/도, {len(all_sigungus_map)}개의 시/군/구, {len(all_eupmyeondong_map)}개의 읍/면/동 맵을 생성했습니다.")
        return sido_map, all_sigungus_map, all_eupmyeondong_map, id_to_region_map
    finally:
        db.close()
...
def create_cache_file():
    ...
    sido_map, sigungus_map, eupmyeondong_map, id_map = get_region_maps_from_db()
    
    combined_maps = {
        "sido_map": sido_map,
        "sigungus_map": sigungus_map,
        "eupmyeondong_map": eupmyeondong_map,
